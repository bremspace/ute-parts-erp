...
- [x] **F3-8 HR Sederhana** ✅ — models (Karyawan, KaryawanKomponenGaji, KomisiTeknisiRule, PayrollPeriode, PayrollSlip, PayrollKomisiDetail) + PayrollService (hitungDraft, ajukanApproval via F1-1, finalisasiDisetujui — guard F1-1 approval, approvePayroll, bayarPayroll) + migration (6 tables, FK fix); Livewire PayrollPage + PayrollSlipDetail + views; route `/app/hr/payroll` (permission `kelola-payroll` — super-admin+finance); COA 520-08 (Beban Komisi) added; HrSeeder + ApprovalRule seeding; tests 6/6 PASS (PayrollServiceTest 4/4, PayrollApprovalTest 2/2); pint clean (2026-09-24); review-fix 2026-09-24: slip `jurnal_id` simpan FK id jurnal (bukan string no_jurnal), teknisi slip sendiri (user_id + cabang), PayrollPermissionTest baru
- [x] **F3-8b Absensi, Shift & KPI** ✅ — migration 5 tables (`shift`, `shift_jadwal`, `absensi_log`, `kpi_metric`, `kpi_hasil`) + jabatan `marketing` (enum MySQL / string SQLite); AbsensiService (clock-in/out idempotent per karyawan+tanggal, status terlambat via shift, overnight shift, tandai izin/cuti, auto-absen, rekap) + KpiService (rumus whitelist: tiket_selesai/transaksi_kasir/selisih_kas/lead_won/penjualan_lead_won — tanpa eval bebas, dari data real, idempotent) + HitungKpiBulananJob (schedule tiap awal bulan); integrasi KasSesiState: buka kas wajib clock-in, tutup kas → clock-out otomatis; RBAC `hr.lihat-sendiri`; Livewire HrAbsensiKpiPage (tab shift/roster/absensi/kpi) + HrSayaPage + routes `/app/hr/absensi` & `/app/hr/saya`; HrSeeder (shift demo, metric KPI, karyawan marketing); fix PayrollService teknisi komisi pakai `user_id`; tests 16/16 PASS (AbsensiShiftTest 6, KpiHitungTest 6, RosterTest 3, 46 assertions) + suite HR 22/22; pint clean (2026-09-24); review-fix 2026-09-24: **potongan disiplin absen §4.2 alur 4 implemented** — `AbsensiService::potonganAbsen` + config `hr.potongan_absen` (threshold, nominal/hari, opt-in cabang; default nonaktif semua) masuk ke total_potongan + rincian slip
- [x] **F3-8c Komisi/Insentif Multi-Aktor** ✅ — engine `KomisiService::hitungKomisiMultiAktor` (PRD §4.3): rule `komisi_skema` (aktor karyawan/reseller/agen × trigger penjualan/lead_won/tiket_servis/target_kpi, nominal/persen, min_amount, cabang_id nullable, is_aktif) + MIGRASI DATA skema reseller lama → seed rule (idempotent, paritas teruji); komisi diperluas (aktor_tipe default `reseller`, aktor_id, komisi_skema_id, lead_id, tiket_servis_id, idempotensi_key unique — idempotent per trigger+rule+aktor; pelanggan_id nullable utk komisi karyawan); pelanggan + `kode_agen` (identitas agen) & `referral_kode` (kode agen yg dipakai pembeli); integrasi trigger: POS/marketplace `penjualan` (engine menggantikan hitungKomisi reseller — paritas sama), `lead_won` (LeadService saat stage→won), `tiket_servis` (ServisService onSelesai — teknisi; reseller servis tetap alur lama), `target_kpi` (HitungKpiBulananJob setelah KPI); komisi karyawan internal masuk payroll (`komisi_internal` di rincian slip, total_komisi = teknisi + internal); rule builder UI `KomisiSkemaPage` `/app/hr/komisi-skema` (kelola-hr) + nav sidebar HR; HrSeeder 4 rule demo; tests 8/8 PASS KomisiMultiAktorTest (rule match multi aktor, idempotent, migrasi paritas, target KPI, payroll); pint clean (2026-09-24); review-fix 2026-09-24: **dedup double-count komisi teknisi** (1 tiket = 1 komisi — engine menang, `hitungKomisiTeknisi` skip tiket yg sudah dicatat engine; `KomisiService` rule-match dedup 3x → helper `queryRuleAktif` tanpa ubah perilaku); HrSeeder jadi 3 rule demo (rule teknisi tiket dihapus — §4.1 KomisiTeknisiRule tetap jalur teknisi)
