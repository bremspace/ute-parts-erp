...
## FASE 11 — Migrasi SID Retail, Optimasi Stok & UX Perbaikan

### T-38 — Optimasi Responsif Global: UI/UX Tanpa Delay di Semua Perangkat
*(Request #11)*
**Modul:** Frontend — seluruh modul (POS, Kasir, CRM, Dashboard, Servis, WMS, Akunting, Reseller)
**Perubahan:** Audit menyeluruh responsivitas & performa UI di breakpoint 320px, 375px, 768px, 1024px, 1440px. Perbaiki:
- Horizontal scroll elimination di semua halaman (container `max-w-screen-xl mx-auto`, `overflow-x-hidden` di root layout)
- Touch target minimum 44×44px (semua button, link, input, dropdown trigger)
- Font rendering optimal: `font-smoothing: antialiased`, `text-rendering: optimizeLegibility`, `font-size` clamp responsive
- Lazy-load gambar & non-critical JS (Vite code-split, `loading="lazy"` di `<img>`)
- Debounce/throttle optimasi: input search 200ms, scroll handler passive, resize observer
- Eliminasi layout shift (CLS): reservasi space untuk skeleton loader, font display swap
- **Acceptance Criteria:**
  - [x] Lighthouse Mobile Performance ≥ 90, Accessibility ≥ 95, Best Practices ≥ 95
  - [x] Zero horizontal scroll di 320px–1440px (Chrome DevTools device toolbar)
  - [x] Semua interaksi tap/click responsif < 100ms (tidak ada delay terasa)
  - [x] CLS < 0.1, FID < 100ms, LCP < 2.5s di halaman kritis (POS, Dashboard, Katalog)

### T-39 — Perbaiki Tombol Logout Sidebar: Arahkan ke `/logout` Web, Bukan `/api/logout`
*(Request #12)*
**Modul:** Auth/UI — `resources/views/layouts/backoffice.blade.php`, `routes/web.php`
**Perubahan:** Tombol logout di sidebar bawah saat ini memanggil `POST /api/logout` (SPA endpoint) yang mengembalikan JSON `{message: "Unauthenticated."}` tanpa redirect UI. Perbaiki:
- Ganti target logout ke `POST /logout` (web route, `Auth\AuthenticatedSessionController::destroy`) yang meredirect ke `/app/login`
- Pastikan CSRF token dikirim (form hidden atau `axios` dengan `X-CSRF-TOKEN`)
- Hapus handler API logout yang tidak dipakai UI (atau biarkan untuk SPA mobile saja)
- **Acceptance Criteria:**
  - [x] Klik logout di sidebar → redirect ke halaman login `/app/login` dengan pesan "Anda telah logout"
  - [x] Session benar-benar dihancurkan (cookie `ute-parts-staging`/`ute-parts` dihapus)
  - [x] Tidak ada alert/error JSON "Unauthenticated." di UI

### T-40 — Perbaiki Alur Stok Masuk: Wajib Via PO Supplier, Terintegrasi Akunting
*(Request #13)*
**Modul:** WMS, Akunting — `app/Modules/Wms/`, `app/Modules/Akunting/`
**Perubahan:** Saat ini user bisa menambah stok langsung dari Master Produk (bypass PO). Wajibkan alur:
- Hapus/tutup fitur "Tambah Stok" manual di Master Produk (hanya admin super-admin boleh via seeder/migrasi)
- Semua penambahan stok **harus** melalui: PO Supplier → `terima-barang` (WMS-11) → jurnal otomatis
- Jurnal PO diterima: `Debit Persediaan (130-01)` / `Kredit Kas (110-01)` tunai ATAU `Kredit Utang Usaha (210-01)` kredit
- PO kredit → buat record `Utang` (modul Akunting) dengan `jatuh_tempo` dari termin supplier
- Pembayaran PO (parsial/penuh) → jurnal `Debit Utang Usaha (210-01)` / `Kredit Kas/Bank (110-01/110-02)` + update sisa utang
- StockMutationLog wajib tercatat di setiap `terima-barang` & `pembayaran` (sumber `po:receive`, `po:payment`)
- **Acceptance Criteria:**
  - [x] Tidak ada UI "Tambah Stok" manual di Master Produk untuk role non-super-admin
  - [x] Setiap stok masuk memiliki PO reference & jurnal balance (SUM debit = kredit)
  - [x] PO kredit muncul di modul Piutang/Utang dengan jatuh tempo benar
  - [x] Riwayat pembayaran PO parsial tercatat, sisa utang terupdate otomatis

### T-41 — Perbaiki Transfer Antar Gudang: Audit Trail, Info Stok Real-time, Validasi Qty
*(Request #14)*
**Modul:** WMS — `app/Modules/Wms/Livewire/WmsDashboard.php`, `resources/views/modules/wms/livewire/wms-dashboard.blade.php`
**Perubahan:** Form pembuatan transfer antar gudang (`createTransfer`) diperluas:
- Tampilkan **stok saat ini di gudang sumber** per item yang dipilih (real-time via `StokItem` query)
- Tampilkan **estimasi total stok baru di gudang tujuan** (stok tujuan + qty transfer)
- Validasi client-side + server-side: `qty_transfer <= stok_tersedia_gudang_sumber` (stok tersedia = stok_fisik - stok_dikunci_pending_transfer)
- Blokir submit jika validasi gagal, pesan error spesifik per item
- Audit trail: setiap transfer mencatat `created_by`, `approved_by`, `received_by`, timestamp lengkap di `StokTransfer` + `StokTransferItem`
- `StockMutationLog` wajib tercatat di `kirim` (sumber `transfer:out`) dan `terima` (sumber `transfer:in`) dengan `referensi_id` = `stok_transfer_id`
- **Acceptance Criteria:**
  - [x] Form transfer menampilkan kolom "Stok Sumber", "Stok Tujuan", "Estimasi Stok Baru Tujuan"
  - [x] Tidak bisa submit transfer melebihi stok tersedia (validasi client & server)
  - [x] Setiap transfer memiliki audit trail lengkap (siapa, kapan, status)
  - [x] StockMutationLog tercatat 2x per transfer (keluar sumber, masuk tujuan) urut `terjadi_at`

### T-42 — Review & Migrasi Skema Database SID Retail (latest.sql) ke Ute Parts
*(Request #15)*
**Modul:** Database, Semua Modul — `database/migrations/`, `app/Modules/**/Models/`
**Perubahan:** File `latest.sql` (backup POS SID Retail) ada di folder project. Lakukan:
- **Review skema**: ekstrak tabel, kolom, FK, index, enum dari `latest.sql` → bandingkan dengan migrasi Ute Parts saat ini (41 migrasi)
- **Gap analysis**: identifikasi entitas SID yang belum ada di Ute Parts (misal: `customer_loyalty_points`, `supplier_price_list`, `serial_number_tracking`, `service_warranty_claim`, `bundle_package`, `promo_campaign`, `employee_shift`, `cash_register_z_report`, dll.)
- **Migrasi adaptif**: buat migrasi baru (prefix `2026_09_21_...`) untuk menambah kolom/tabel yang hilang, **tanpa breaking existing** (nullable/default, FK optional)
- **Seeder migrasi data**: buat `SidRetailMigrationSeeder` yang `LOAD DATA INFILE` / `INSERT ... SELECT` dari dump SQL (staging) ke tabel Ute Parts dengan mapping kolom
- **Sinkronisasi arsitektur**: pastikan relasi `Produk`–`SkuVariant`–`SatuanUnit`–`Rak`–`Gudang` mengakomodasi struktur SID (multi-satuan, serial number, batch/expiry)
- **Acceptance Criteria:**
  - [x] Dokumen `MIGRATION_SID_RETAIL.md` berisi gap analysis + mapping tabel-kolom
  - [x] Migrasi baru diterapkan tanpa error (`php artisan migrate --force` di staging)
  - [x] Seeder migrasi data jalan idempotent, data SID masuk ke tabel Ute Parts benar
  - [x] Semua modul (POS, Servis, WMS, CRM, Akunting) berfungsi dengan data migrasi SID
  - [x] Tidak ada data loss: jumlah produk, pelanggan, supplier, transaksi SID = Ute Parts pasca-migrasi

### T-43 — Batch Upload Produk via Excel: Template, Validasi, Auto-import Tanpa Missing Data
*(Request #16)*
**Modul:** WMS — `app/Modules/Wms/Jobs/ImportProdukExcelJob.php`, `resources/views/modules/wms/livewire/wms-dashboard.blade.php`
**Perubahan:** Tambah fitur "Import Produk Excel" di Master Produk:
- Template Excel (`.xlsx`) downloadable: kolom `sku`, `nama`, `barcode`, `satuan`, `kategori`, `tipe_hp`, `brand`, `kualitas`, `harga_beli`, `harga_jual`, `harga_reseller`, `harga_agen`, `stok_awal`, `gudang_id`, `rak_id`, `kompatibilitas_hp` (JSON string), `foto_url` (opsional)
- Upload: validasi header wajib, cek duplikat SKU/barcode, validasi FK (kategori, satuan, gudang, rak, brand)
- Proses via **queue job** (`ImportProdukExcelJob`, `ShouldQueue`, chunk 100 baris) → tidak timeout RAM 1GB
- Response: preview 5 baris pertama sebelum commit (dry-run), lalu tombol "Konfirmasi Import"
- Hasil import: log sukses/gagal per baris, `StockMutationLog` sumber `import:excel`, jurnal stok awal `Debit 130-01 / Kredit 310-01` (modal stok awal)
- **Acceptance Criteria:**
  - [x] Template Excel benar & lengkap (download di UI)
  - [x] Upload 1000+ baris tidak timeout (queue + chunk)
  - [x] Dry-run preview menampilkan error validasi per baris (SKU duplikat, FK tidak ditemukan, dll.)
  - [x] Import commit → data masuk database benar, jurnal stok awal balance, StockMutationLog tercatat
  - [x] Tidak ada missing data: semua kolom template termapping ke kolom DB

### T-44 — Database Produk Komprehensif: Barcode, Satuan, Kategori, HP, Brand, Kualitas, Multi-tier Harga, CRM Sync
*(Request #17)*
**Modul:** WMS, CRM, POS — `app/Modules/Wms/Models/Produk.php`, `SkuVariant.php`, `SatuanUnit.php`, `KategoriProduk.php`, `Brand.php`, `KualitasProduk.php`, `HargaTier.php`
**Perubahan:** Perluas entitas produk agar mengakomodasi data SID Retail & kebutuhan bisnis:
- `Produk`: `barcode` (unique), `satuan_id` (FK `SatuanUnit`), `kategori_id`, `tipe_hp` (enum/array: iPhone, Samsung, Xiaomi, dll.), `brand_id` (FK `Brand`), `kualitas_id` (FK `KualitasProduk`: Original, Grade A, Grade B, Refurbished, Compatible)
- `SkuVariant`: `harga_jual` (base), `harga_reseller`, `harga_agen`, `harga_grosir` (multi-tier)
- `HargaTier` baru: `pelanggan_tier_id` (FK `TierMembership`), `tipe_konsumen` (retail/reseller/agen), `persen_diskon` / `nominal_tetap` → resolve harga dinamis di POS/CRM
- `KompatibilitasHP` relasi many-to-many (produk ↔ tipe HP terstruktur, bukan string bebas)
- **CRM Sync**: `PelangganService::resolveHarga(produk, pelanggan)` memakai `HargaTier` + `tipe_konsumen` pelanggan (retail/reseller/agen) → satu sumber harga
- Form Master Produk: lengkapi semua field di atas, validasi wajib (barcode unique, satuan wajib, minimal 1 harga tier)
- **Acceptance Criteria:**
  - [x] Produk memiliki barcode, satuan, kategori, tipe HP, brand, kualitas, multi-tier harga
  - [x] Harga otomatis resolve di POS/CRM berdasarkan tier pelanggan & tipe konsumen
  - [x] Data SID Retail (latest.sql) termapping penuh ke skema baru
  - [x] Form CRUD produk lengkap, validasi ketat, tidak ada field kosong wajib

### T-45 — Perbaiki Buka Kas Saat Login Backoffice: Separator Ribuan, Submit Response, Error Handling
*(Request #18)*
**Modul:** POS, KasSesi — `app/Modules/Pos/Livewire/PosKasir.php`, `resources/views/modules/pos/livewire/pos-kasir.blade.php`
**Perubahan:** Saat login backoffice, diarahkan ke modal "Buka Kas" dengan input nominal:
- **Separator ribuan**: gunakan komponen `x-format-number` (Alpine directive `format-number` dari T-34) di input `saldo_awal` — pastikan `toRaw()` mengirim angka bersih ke backend
- **Submit response**: `bukaKas()` harus return feedback (toast sukses/error), redirect ke POS dashboard, **bukan diam** (cek `wire:loading`, `wire:target`)
- **Error handling**: validasi nominal > 0, numeric, maksimal batas wajar; tampilkan error inline + toast
- **Auto-focus** input nominal saat modal buka; `Enter` submit form
- **Acceptance Criteria:**
  - [x] Input nominal menampilkan separator ribuan real-time (1.000.000)
  - [x] Submit nominal valid → toast "Kas berhasil dibuka", redirect ke POS, sesi kas status `buka`
  - [x] Submit nominal invalid (kosong, negatif, non-numeric) → error inline + toast, modal tetap terbuka
  - [x] Backend menerima angka bersih (tanpa separator), jurnal `Debit 110-01 / Kredit 310-01` balance

### T-46 — Perbaiki Sidebar Hilang di PC/Tablet: Layout Responsif Sidebar
*(Request #19)*
**Modul:** Frontend — `resources/views/layouts/backoffice.blade.php`, `resources/css/app.css`
**Perubahan:** Sidebar mengilang di viewport ≥ 768px (tablet/PC). Root cause kemungkinan: Tailwind `lg:hidden` di sidebar wrapper atau `lg:translate-x-0` tidak aktif. Perbaiki:
- Sidebar **selalu terlihat** di `lg:` (≥ 1024px) dan `md:` (≥ 768px) — gunakan `hidden lg:block` pattern yang benar
- Mobile (< 768px): sidebar sebagai drawer (slide-in), overlay backdrop, tombol hamburger
- Tablet (768px–1023px): sidebar collapse/expand toggle (ikon-only ↔ full), default expanded
- Desktop (≥ 1024px): sidebar full expanded, fixed, tidak collapse otomatis
- Z-index management: sidebar > header > content, drawer overlay di mobile
- **Acceptance Criteria:**
  - [x] Sidebar terlihat normal di PC (≥ 1024px) & tablet (≥ 768px) tanpa toggle
  - [x] Mobile (< 768px): hamburger buka drawer, klik overlay/klik item tutup drawer
  - [x] Tablet: toggle collapse/expand berfungsi, state tersimpan (localStorage)
  - [x] Tidak ada layout shift / content tertutup sidebar

### T-47 — Perbaiki Error Dashboard: Column 'sisa' Not Found di Tabel `piutang`
*(Request #20)*
**Modul:** Akunting, Dashboard — `app/Modules/Akunting/Models/Piutang.php`, `database/migrations/`, `app/Modules/Dashboard/Livewire/DashboardIndex.php`
**Perubahan:** Error `SQLSTATE[42S22]: Column not found: 1054 Unknown column 'sisa' in 'field list'` pada query `sum(sisa) from piutang where status != lunas`:
- Cek migrasi tabel `piutang` (seharusnya kolom `sisa` atau `sisa_piutang` atau `jumlah - dibayar` computed)
- Jika kolom `sisa` tidak ada: tambah migrasi `2026_09_21_..._add_sisa_to_piutang_table` (kolom `sisa` decimal 20,2 default 0) + update existing rows `sisa = jumlah - dibayar`
- Atau gunakan accessor `getSisaAttribute()` di model `Piutang` (computed: `jumlah - dibayar`) dan query pakai `DB::raw('SUM(jumlah - dibayar)')`
- Perbaiki `DashboardIndex::chartPiutangAging()` & semua query yang pakai `sisa` → pakai computed/accessor
- **Acceptance Criteria:**
  - [x] Query dashboard piutang aging jalan tanpa error
